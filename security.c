...
int send_public_key(int sockfd,int index){
	...
	for(i=1;i<my_addr;i++){
		...
		key_socket=setup_key_socket(index);
		...
		bytes_written=sendto(sockfd,packet,gyn.pkt_size+sizeof(struct gyn_header),0,(struct sockaddr*)&sockaddrll,sizeof(struct sockaddr_ll));
		if(bytes_written<0){
